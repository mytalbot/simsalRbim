...
Data from a preference test may be incomplete, may show ties or new item 
positions shall be tested without the need of testing ALL possible combinations.
The simsalRbim package can address all three cases by using informed and 
uninformed simulations to find the optimal item positions in the data.

Preference test data usually come in two formats: quantities or binary data. The
bimload function in the package can load both formats, including the preferred
side in a test as a variable. However, if a preference test is showing
significant side dependencies, the simulated outcome will be biased. With the
current package it is not possible to resolve side-dependent tests.

In case of present ties, the binary response variable is randomized (by default, 
this happens at a 50% Likelihood for an equal commodity selection. However, the
user may specify other thresholds or quantities). Missing item tests and 
combinations are filled as ties and then simulated in the two simulation
functions. In the uninformed simulation, data are continuously randomized and 
analyzed. At a given number of degrees of freedom this will stabilize the 95%
confidence intervals on the worth scale. Thus, item positions can be obtained.
In the informed simulation, the item intransitivity is also calculated. By 
excluding "intransitive" selections from the results, the item of interest can
...
Please note, that the item 'HoiHoiHoi' in the data introduces a tie and 
additional variance. Without it, this would be perfect data. Such data do not 
need a simulation. In such cases, the package will show the worth plot but will
also give a warning.
...
of the tie selection (e.g, 50% + deviation). 
...
showPlot=FALSE,ylim=c(0, 0.7),verbose=FALSE)
```
...
(present!) ties are randomized each time the function is executed. The `intrans`
...
bimeval(ydata=NULL,GT=NULL,simOpt=NULL,worth= NULL,coverage=0.8,showPlot=TRUE )
```
...
defines an arbitrary threshold for data coverage warnings. There can be two 
warnings: number of subjects and number of items warnings. The graphical output
can be controlled with the `showPlot` object. 
...
arguments. However, it will not explore randomization space but will rather use
a distinct cutoff to show the items' relative position. The query item 
(`simOpt`) is shown in "red" in the plot. All items will be shown on the worth
scale, together with 95% confidence invervals. Any overlap will indicate
potential ambiguity in item positioning.


### bimsim 
```
# bimsim - informed position simulation
bimsim(ydata=NULL,GT=NULL,simOpt=NULL,limitToRun=10,seed=TRUE,fval=0.9,
showPlot=TRUE,ylim=c(0, 0.7))
...
can be used as an estimate for the required number of randomizations. The `fval`
argument sets the cutoff for showing the intransitivity ratio (Iratio) of the 
results in the plot. fval=1 means that 100% of the intransitive results are 
plotted. The IRatio is calculated as the (number of triplets / no. of
intransitive triplets) * 100. The more stringent (= lower fval) the criterion,
the more informed the simulation becomes. The output is a frequency distribution
of `simOpts` preferred position at the given `fval`. The plot shows the worth 
value of the `simOpt` item as a function of its position. The bubblesize codes 
for the IRatio.
