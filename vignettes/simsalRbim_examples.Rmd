...
but one (‘HoiHoiHoi’) that was tested with only 4 subjects. In addition, there
...
function to preprocess the data into a unified format `bimpre` also identifies 
ties. As you can see from the example below, the ties get marked in the ‘tie’ 
column. 
...
and transitive choices are considered. The intransitivity is expressed as the
consensus error (CE) at each randomization step.
This way, we can estimate which choices were inconsistent and include this in
our evaluation. We use a frequency distribution to decide on the most likely 
position of items.  
...
The table shows, that the items with a low consensus error, i.e.,
