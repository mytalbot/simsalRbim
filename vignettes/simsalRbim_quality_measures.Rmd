...
A, B and ties there were and multiplies this with 1/No. of subjects.  

In the example of the highlighted case above, this would be  
No. B x 1/4 No. of subjects = 2 x B x 0.25 = 0.5

Ties were attributed equally on options A and B to avoid distribution bias. The
most simple solution requires adding half of the count values to A AND B. This 
will be:  

(No. of B x 1/4 No. of subjects) + ((No. Ties x 1/4 No. of subjects)/2) =
(2 x 0.5) + ((2x0.25)/2) = 0.75.

*AND*  

(No. of A x 1/4 No. of subjects) + ((No. Ties x 1/4 No. of subjects)/2) =
0 + ((2x0.25)/2) = 0.25.

This is repeated with all item combinations, resulting in raw deviations
for options A and B (CE_A and CE_B). The following table shows the calculations
weighted by the number of subjects as the raw consensus errors.
...
for(i in 1:item_combinations){ # e.g., ZackeHoiHoiHoi
  if( item[i, "CE_A" ] > item[i, "CE_B" ] ){
    delta  <- (50 - item[i, "CE_B" ])
  }else{
    delta  <- (50 - item[i, "CE_A" ])
  }
}
```

With the resulting delta value, the deviation from 50% can be calcualted. A 
...
The detailed error table is summarized for each item in the test by reporting 
the average CE together with the worth value. The final error table is  
...
of transitivity: If one item, A, is preferred over another item, B, and B is 
preferred over item C, then A should also be preferred when directly compared to
...
Let's say, there are 40 triplets in total and 3 triplets showed intransitive
patterns. Then, the intransitivity ratio would be **Iratio = 3/40 = 0.075**.

Including simulated ties can introduce intransitivity or help with improving 
...
combinations. Usually, the best choice for a new item will be the one with the
