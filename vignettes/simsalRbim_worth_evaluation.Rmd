...
Let’s say, we want to assess the consensus error of the newly entered 
...
in these cases, while natural ties still show quantities).
...
Example: ‘Zacke’ vs ‘HoiHoiHoi’. The table above shows that from four subjects 
only one (‘drei’) preferred the item ‘Zacke’ over item ‘HoiHoiHoi’, result = 1.
The other subjects prefered ‘HoiHoiHoi’ over ‘Zacke’, result = -1. This is
converted into percentages, so that 25% of the total subjects preferred option A
(‘Zacke’) and 75% option B (‘HoiHoiHoi’).
...
In the example ‘Huehner’ vs ‘Kacke’, 100% of the subjects preferred option A 
(‘Huehner’), etc.
...
The closer the resulting value is to 50, the more agreement there was between 
subjects on any given item.

The delta is then averaged, standardized, and subtracted from 50%, resulting
...
The mean delta of all 'HoiHoiHoi' items is 39.06%:
((50+50+50+50) + (25+25+25+50)+ (50+50+50+50) + (25+25+25+25))/16 = 625/16 = 
39.06%.

Then we calculate 50%-39.06%=10.94% deviation. This is standardized to 50,
resulting in a CE of 10.94%/50*100=21.88% which is also shown in the following
plot.

Therefore, the item ‘HoiHoiHoi’ shows a 21.88% consensus error indicating that
21.88% of test subjects disagreed on the positioning of the item. This is also 
